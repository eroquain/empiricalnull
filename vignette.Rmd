...
data(hivdata)
Yhiv=hivdata
nbrejectionshiv=BH(Yhiv,alpha=0.1)
...
hist(Yhiv,nclass=70,ylim=c(0,1.6/sqrt(2*pi)),freq=FALSE,main="",xlab="van't Wout et al. (2003)",ylab="")
curve(dnorm(x),lwd=2,add=TRUE)
hist(Yhiv,nclass=70,freq=FALSE,add=TRUE)
hist(YALL,nclass=70,ylim=c(0,1.6/sqrt(2*pi)),freq=FALSE,main="",xlab="Bourgon et al (2010)",ylab="")
curve(dnorm(x),lwd=2,add=TRUE)
hist(YALL,nclass=70,freq=FALSE,add=TRUE)
```
...
tab <- data.frame("Data" = c("Golub","Heden", "HIV","ALL"), 
                  ...
                  getpvalueF0(alpharange,pnorm,pibar,Yheden), getpvalueF0(alpharange,pnorm,pibar,Yhiv),
                  getpvalueF0(alpharange,pnorm,pibar,YALL)
...
nbrejethiv=plotempfit(Yhiv)
nbrejetYALL=plotempfit(YALL)
```
...
tab <- data.frame("Data" = c("Golub","Heden", "HIV","ALL"), 
                  "theoretical BH procedure" = c(nbrejetgolub[1],nbrejetheden[1], nbrejethiv[1],nbrejetYALL[1]),
                  "plug-in BH procedure" = c(nbrejetgolub[2],nbrejetheden[2], nbrejethiv[2],nbrejetYALL[2]),check.names = FALSE, row.names = NULL)
knitr::kable(tab)
```

It is apparent that the discovered variables highly depend on the plugged null distribution and that the theoretical BH procedure could generate a lot of false discoveries (or false non-discoveries). This reinforces the interest in suitably estimating the null before applying the BH procedure.


## Label-permuted empirical null

In (multiple) testing, a popular method for estimating the null is to use permutations of the labels. Let us investigate how this compares to the previous method. 
...
# Practical recommandations: a new confidence region for the null
...
However, this minimax result is pessimistic as, for other distributions, plug-in could still be possible. This raises the challenge of assessing the performance of plug-in for the specific distribution of the data.

Given these facts, what could be a practical recommandation for the user? 

Keeping the assumption that the null distribution is Gaussian $\mathcal{N}(\theta,\sigma^2)$, one option is too draw a confidence map for the confidence region described above and given in Section 6 of [@RV2019]. For this, we report all the parameters $(\theta,\sigma^2)$ such that $F_0=\mathcal{N}(\theta,\sigma^2)$ is inside the confidence region with coverage, say, $90\%$. To get more insight, we can display at each point $(\theta,\sigma^2)$ of the region, the number of rejections of the corresponding plug-in BH procedure.


```{r computing parameter confidence region }
getBHrejectionsCR=function(Y,alpha,pibar,murange,sigmarange){
  N=length(murange)
Admis=matrix(0,N,N)
for (i in 1:N){
  for (j in 1:N){
    Admis[i,j]=NA
    F0=function(x) pnorm((x-murange[i])/sigmarange[j])
    if(acceptF0(alpha,F0,pibar,Y)){
      Admis[i,j]= BH((Y-murange[i])/sigmarange[j],alpha=0.1)
    }
  }
}
return(Admis)
...
plotconfidenceregion=function(Y){
  ...
  sigmatilde=getsigmatilde(Y)
  N=13
  murange=seq(thetatilde-0.6,thetatilde+0.6,length.out=N)
  sigmarange=seq(sigmatilde-0.6,sigmatilde+0.6,length.out=N)
  Admis=getBHrejectionsCR(Y,alpha=0.1,pibar=0.1,murange,sigmarange)
  ...
  axis.col=axis(side=2,labels=signif(murange,2),at=1:N),line=1,main="")
}

#par(mfrow=c(2,2))
nbrejetgolub=plotconfidenceregion(Ygolub)
nbrejetheden=plotconfidenceregion(Yheden)
nbrejethiv=plotconfidenceregion(Yhiv)
nbrejetYALL=plotconfidenceregion(YALL)
...
Following these recommandations on the data sets considered above, the user thus might be cautious for the data @Golub99 and @Hed2001, while might declare many more findings for the data @van2003 and @BGH2010.
